...
.factory('srvData', function ($rootScope,$q,$resource,$log,$http, appName,appVersion,appServerUrlLocal,appServerUrlRemote) {
  return new SrvDataPouchDB($rootScope,$q,$resource,$log,$http,appName,appVersion,appServerUrlLocal,appServerUrlRemote);
});


var SrvDataPouchDB = (function() {
  function Service($rootScope,$q,$resource,$log,$http,appName,appVersion,appServerUrlLocal,appServerUrlRemote) {
      ...
      this.appServerUrlLocal=appServerUrlLocal;
      this.appServerUrlRemote=appServerUrlRemote;
      this.serverUrl = this.appServerUrlLocal;
  ...
  Service.prototype.testConnection = function () {
    ...
    self.$http.get(self.appServerUrlLocal)
    .success(function(data) {
      self.serverUrl = self.appServerUrlLocal;
      deferred.resolve(self.serverUrl);
    })
    .error(function(data) {
      self.serverUrl = self.appServerUrlRemote;
      return deferred.reject(self.serverUrl);
    });
  ...
  Service.prototype.init = function () {
      ...
      this.db = new PouchDB('myAppDB', {adapter : 'websql'});
      if (!this.db || !bok) return;

      // Test Server Url
      self.testConnection().then(function(url) {
        self.initDone = true;
  ...
  Service.prototype.becarefulClean = function() {
    ...
    self.db.destroy(function(err, info) {
      if(!err) {
        self.db = new PouchDB('myAppDB', {adapter : 'websql'});
        deferred.resolve();
      }
    });
