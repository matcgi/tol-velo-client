...
.controller('DashboardCtrl', function ($scope, $timeout, $routeParams, $q, srvData, srvConfig) {
	...
	$scope.dashboardShowLastHistoricDate = function(historic) {
		...
		var choreId = historic[$scope.appHistoricCols.choreId];
	...
	$scope.dashboardTerminateHistoric = function(historic) {
		...
		if (i >= 0) {
			...
			historicToAdd[$scope.appHistoricCols.choreId] = historic[$scope.appHistoricCols.choreId];
			historicToAdd[$scope.appHistoricCols.userId] = $scope.dashboardSearch[$scope.appHistoricCols.userId];
			historicToAdd[$scope.appHistoricCols.frequencyDays] = padInteger(historicToAdd[$scope.appHistoricCols.frequencyDays]);
		}
	...
	$scope.dashboardDisplayHistoricCalendar = function(dateAsYYMMDD){
		var display = "" + $scope.dashboardDisplayHistoricDate(dateAsYYMMDD) + " "+dateAsYYMMDD;
		return display;
	};
	...
	$scope.dashboardComputeDoneIndicators = function() {
		...
		for (var k = 0; k < $scope.dashboardHistoricsDone.length; k++) {
			...
			var userIndex = (historic[$scope.appHistoricCols.userId] == $scope.userA._id) ? 1 : 0;
	...
	$scope.dashboardComputeIndicleanators = function(showLabels) {
		...
		for (var i = 0; i < $scope.dashboardHistoricsDone.length; i++) {
			var historic = $scope.dashboardHistoricsDone[i];
			if (historic[$scope.appHistoricCols.userId] == $scope.userA._id) {
				userATimeSpent += historic[$scope.appHistoricCols.timeInMn];
			}
			else {
				userBTimeSpent += historic[$scope.appHistoricCols.timeInMn];
			}
		}
